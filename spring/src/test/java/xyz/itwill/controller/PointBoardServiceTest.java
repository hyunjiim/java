...
// => Spring Bean Configuration File의 AOP 설정을 이용하여 TransactionManager 객체를 사용하여
//트렌젝션 처리를 할 수 있지만 @Transactional 어노테이션을 사용하여 트렌젝션 처리 가능

//TransactionManager 객체를 이용하여 트렌젝션 처리 기능을 제공받을 메소드에 @Transactional
//어노테이션 사용하면 메소드의 명령 실행시 예외(Exception)가 발생된 경우 자동으로 롤백 처리
// => @Transactional 어노테이션을 사용하기 위해 Spring Bean Configuration File(root-context.xml)에
//tx 네임스페이스를 추가하여 spring-tx.xsd 파일을 제공받아 annotation-driven 엘리먼트를 반드시 설정
// => 테스트 클래스의 테스트 메소드에 @Transactional 어노테이션을 사용하면 테스트 메소드의
//명령 실행 후 무조건 롤백 처리
...
public class PointBoardServiceTest {
	...
	public void test1() throws Exception {
		...
		//PointBoardService 클래스의 getPointBoardList() 메소드를 호출하여 게시글 목록을 반환받아 [주석]
	...
	public void test2() throws Exception {
		//PointBoardService 클래스의 removePointBoard() 메소드를 호출하여 POINTBOARD 테이블에 게시글 삭제 처리
		// => POINTUSER 테이블에 저장된 회원정보 중 게시글 작성자에 대한 회원정보의 포인트 감소
		// => POINTUSER 테이블에 저장된 회원정보 중 게시글 작성자의 회원정보를 검색하여 반환
		PointUser user=pointBoardService.removePointBoard(1); //게시글 삭제 처리
