...
// => SpringMVC 프로그램에서 사용하는 모듈 : DAO 클래스, Service 클래스, Controller 클래스
...
//3.[src/text/java] 폴더에 테스트 프로그램 관련 클래스 작성
// => juinit 라이브러리와 spring-test 라이브러리는 test 프로그램을 실행할 경우에만 빌드 처리
// => 프로그램 작성시 자동완성기능을 사용하지 못함
// => junit 라이브러리와 spring-test 라이브러리의 scope 속성을 주석 처리한 경우 테스트 프로그램
//관련 클래스 작성- 테스트 프로그램 관련 클래스 작성 후 테스트 프로그램을 실행할 때 scope 주석 제거
//4. JUnit 테스트 프로그램을 실행하여 모듈 검사
...
//@ContextConfiguration : 테스트 프로그램에서 사용될 Spring Bean을 제공하기 위한 Spring Bean 
...
public class DataSourceTest {
	...
	// => SpringJUnit4ClassRunner 클래스에 의해 테스트 프로그램 관련 클래스가 객체로 생성된 후 자동으로 호출되는 메소드
	@Test
