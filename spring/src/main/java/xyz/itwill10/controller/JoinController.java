...
public class JoinController {
	...
	/*
	//전달값을 주고받는 중요한 방법 - 전달값이 1, 2개일 경우 사용
	...
	*/
	...
	// => @ModelAttribute 어노테이션을 [메소드]에 사용하면 메소드의 반환값을 요청 처리 클래스의
	//모든 요청 처리 메소드의 뷰에게 제공
	// => @ModelAttribute 어노테이션을 [매개변수]에 사용하면 매개변수에 저장된 값을 현재 요청
	//처리 메소드의 뷰에게 제공 - Model 객체를 사용하지 않고 뷰에게 필요한 객체(값)를 속성값으로 제공
	//value 속성 : 뷰에서 속성값을 사용하기 위한 속성명을 속성값으로 설정
	// => 다른 속성이 없는 경우 속성값만 설정 가능
	@RequestMapping(value = "/join", method = RequestMethod.POST)
	public String join(@ModelAttribute(value="id") String id, @ModelAttribute(value="passwd") String passwd
			,@ModelAttribute(value="name") String name, @ModelAttribute(value="email") String email) {
		return "join_display";
	}
	*/
	...
	*/
	...
	/*
	//전달값을 주고받는 두번째 중요한 방법 - 전달값이 2개 이상일 때 사용 : Command 객체로 전달
	...
	//VO(DTO) 클래스의 객체를 저장
	//Command 객체 : 전달값을 제공받아 필드에 저장된 객체로 속성값이 저장되어 요청 처리
	...
	*/
	...
	@RequestMapping(value= "/join", method = RequestMethod.POST)
	public String join(@ModelAttribute("mem") Member memebr, Model model) {
		if(memebr.getId().equals("abc123")) {//아이디가 중복된 경우
