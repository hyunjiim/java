...
public class UserinfoController {
	...
	*/
	...
	//인터셉터(Interceptor) : 요청 처리 메소드가 호출되기 전 또는 후에 삽입되어 실행될 명령을 제공하는 기능
	...
	*/
	...
	//회원정보를 전달받아 USERINFO 테이블에 삽입하고 로그인 페이지를 요청할 수 있는 URL 주소를 클라이언트에게
	//전달하여 응답 처리하는 요청 처리 메소드
	// => UserinfoService 객체오 메소드 호출시 예외 발생 - 예외 처리 메소드를 이용하여 예외 처리
	@RequestMapping(value = "/write", method = RequestMethod.POST)
	...
	*/
	...
	/*
	//USERINFO 테이블에 저장된 모든 회원정보를 검색하여 속성값으로 저장하며 회원목록을 출력하는
	//뷰이름을 반환하는 요청 처리 메소드
	...
	public String list(Model model, HttpSession session) throws BadRequestException {
		...
		if(loginUserinfo == null) {
			throw new BadRequestException("비정상적인 요처입니다.");
		}
