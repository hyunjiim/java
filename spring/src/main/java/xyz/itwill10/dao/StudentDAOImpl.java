...
// => [src/main/webapp] 폴더에 작성해야만 스프링 컨테이너(WebApplicationContext 객체 - 웹에 대한 자원만)가
...
//File에서 bean 엘리먼트로 클래스를 Spring Bean 등록 - root-context.xml(모든 Front Controller가 객체를 사용하고 싶을 경우) 또는 servlet-context.xml(웹서블러?라는 이름의 Controller 파일)
...
//Spring Bean Configuration File(servlet-context.xml 파일)의 context:component-scan 엘리먼트로 검색되도록 설정
@Repository
