...
// => 사람마다 salt가 달라 실제 같은 문자열을 암호화 처리될 때 결과물도 다름
// => BCrypt 클래스 : 단방향 암호화(복호화 기능 없음) 기능의 BlowFish 알고리즘을 기반으로 설계된 암호화 처리 클래스
...
public class UserinfoServiceImpl implements UserinfoService {
	...
	public void addUserinfo(Userinfo userinfo) throws ExistsUserinfoException {
		...
		if(userinfoDAO.selectUserinfo(userinfo.getUserid()) != null) {
			//예외를 명확하게 구분하고 예외처리시 필요한 값을 제공받기 위해 직접 생성한 예외
			//클래스를 사용하여 예외 발생
			throw new ExistsUserinfoException("이미 사용중인 아이디를 입력하였습니다.", userinfo);
		}
	...
	// => 인증 성공시 로그인 처리된 회원정보를 검색하여 반환 - 세션에 권한 관련 정보로 저장
	@Override
