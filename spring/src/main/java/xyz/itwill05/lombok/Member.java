...
//@NoArgsConstructor : 매개변수가 없는 생성자를 자동 생성하여 제공하는 어노테이션- Member()
//@AllArgsConstructor : 모든 필드에 초기값을 저장할 수 있는 매개변수가 있는 생성자를 제공하는 어노테이션 - Member(String,String,String)
// => 모든 필드에 초기값을 저장할 수 있는 매개변수가 있는 생성자 생성
//@RequiredArgsConstructor : 매개변수가 있는 생성자를 자동으로 생성하여 제공하는 어노테이션  
// => final 제한자를 사용하여 선언된 필드에 초기값을 저장할 수 있는 매개변수가 있는 생성자 생성
// => @NoArgsConstructor 어노테이션과 같이 사용 불가
...
//@Builder 어노테이션은 Test 프로그램을 작성할 때 사용하면 유용
//@Builder : 클래스에 Builder 클래스를 자동 생성하여 제공하는 어노테이션
// => Builder 클래스 : 객체 생성시 객체 필드에 필요한 값을 저장하기 위한 메소드를 제공하는 클래스
// => 생성자 보다 가독성이 좋으며 필드의 순서에 상관없이 초기화 작업 편리
// => 매개변수가 있는 생성자에 @Builder 어노테이션을 사용하여 생성자 매개변수로 초기화 처리하기 
//위한 필드에만 값을 저장할 수 있는 메소드 제공

//@Slf4j : 로그 이벤트를 발생하는 Logger 객체가 저장된 필드(로그)를 자동 생성하여 제공하는 어노테이션  
