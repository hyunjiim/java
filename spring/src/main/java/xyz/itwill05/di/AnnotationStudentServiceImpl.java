...
public class AnnotationStudentServiceImpl implements StudentService {
	...
	//@Autowired : 스프링 컨테이너로부터 Spring Bean을 제공받아 필드에 저장하여 의존관계를
	...
	//문제점) 필드의 자료형과 같은 자료형의 Spring Bean이 2개 이상인 경우 의존성 주입 실패 - NoUniqueBeanDefinitionException
	//해결법-1) 필드의 자료형과 같은 자료형의 Spring Bean이 2개 이상인 경우 필드에 저장될 Spring Bean의
	//식별자(beanName)을 필드명과 동일하게 변경
	// => @Autowired 어노테이션은 필드의 자료형과 같은 자료형의 Spring Bean이 2개 이상인 경우
	//autowire 속성값을 [byName]으로 설정한 것과 동일한 방법으로 의존성 주입
	//해결법-2)필드의 자료형과 같은 자료형의 Spring Bean이 2개 이상인 경우 필드에 저장될 Spring Bean의
	//클래스에 @Primary 어노테이션을 사용하여 의존성 주입
	//해결법-3)@Autowired 어노테이션에 종속된 @Qualifier 어노테이션을 사용하여 의존성 주입
	//@Qualifier : 필드에 저장될 Spring Bean을 직접 지정하여 의존성 주입
