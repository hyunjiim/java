...
public class HewonAdvice {
	...
	//타겟 메소드(Target Method) : 핵심관심모듈의 메소드 중 PointCut 표현식으로 지정되어 횡단관심코드가 삽입될 메소드
	...
	//타겟메소드의 명령이 정상적으로 실행된 후 삽입되어 실행될 명려을 작성한 메소드 - After Returning Advice Method
	public void afterReturningLog() {
		System.out.println("### [after-returning]핵심관심코가 정상적으로 실행된 후 삽입되어 실행될 횡단관심코드 ###");
	}
	
	//타겟메소드의 명령 실행시 예외가 발생한 후 삽입되어 실행될 명령을 작성한 메소드 - After Throwing Advice Method
	public void afterThrowingLog() {
		System.out.println("### [after-throwing]핵심관심코 실행시 예외가 발생된 경우 삽입되어 실행될 횡단관심코드 ###");
	}
