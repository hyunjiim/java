...
public class JoinPointAdvice {
	...
	public void beforeDisplay(JoinPoint joinPoint) {
		...
		//JoinPoint.getSignature() : 타겟 메소드의 정보가 저장된 Signature 객체(리턴타입, 이름, 매개변수)를 반환하는 메소드
	...
	public void displayMessage(JoinPoint joinPoint) {
		System.out.println("### [after]핵심관심코드 실행 후 무조건 삽입되어 시행될 횡단관심코드 ###");
		
		Object[] params=joinPoint.getArgs();
		System.out.println("### [after]사원번호가 "+params[0]+"인 사원정보를 삭제 하였습니다. ###");
	}
	
	//After Returning Advice Method - 반환형 : void, 매개변수: 타겟메소드에서 반환되는 객체/값(Object)
	// => JoinPoint 인터페이스의 매개변수 외에 Object 클래스의 매개변수 선언 가능
	// => 스프링 컨테이너는 Object 클래스의 매개변수에 타겟 메소드의 반환값을 제공받아 전달
	// => 꼭 [Object] 자료형을 사용해야 하는건가? - 타겟메소드에세 반환되는 값(객체)의 자료형이 고정되어
