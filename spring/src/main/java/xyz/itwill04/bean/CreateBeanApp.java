...
public class CreateBeanApp {
	public static void main(String[] args) {
		...
		//BeanFactory.getBean(String beanName) : 매개변수로 Spring Bean을 구분하기 위한 식별자(beanName)
		//를 전달받아 스프링 컨테이너로부터 Spring Bean(객체)를 생성하여 반환하는 메소드
		// => Object 타입의 객체를 반환하므로 반드시 명시적 객체 형변환 사용
		// => 매개변수로 전달받은 식별자(beanName)의 Spring Bean이 없는 경우 NoSurchBeanDefinitionException 발생
		...
		//식별자(beanName)를 전달받아 스프링 컨테이너로부터 Spring Bean(객체)을 반환하는 메소드
		// => Object 타입의 객체를 반환하므로 반드시 명시전 객체 형변환 사용
