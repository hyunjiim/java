...
public class BeanAttributeApp {
	public static void main(String[] args) {
		...
		//beanName의 Spring Bean(객체)를 검색하여 반환하는 메소드 - DL
		...
		//매개변수로 전달받은 beanName의 Spring Bean을 Class 객체로 객체 형변환하여 반환하는 메소드
		InitDestroyMethodBean bean=context.getBean("initDestroyMethodBean", InitDestroyMethodBean.class);
		
		//bean 엘리먼트를 이용하면 초기화 메소드를 생성한 후 자동으로 호출되도록 설정 가능
		//bean.init(); //초기화 메소드
