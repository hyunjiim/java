...
//형식)package 패키지경로;
//=>Java 자료형이 어떤 패키지에 선언된 자료형인지를 명확하게 구분하여 사용

//같은 패키지에 작성된 클래스는 패키지 경로 표현없이 클래스만 사용하여 접근이 가능
...
//=>형식) import 패키지경로.자료형(클래스/인터페이스/열거형);
//=>package 키워드로 작성된 명령 하단에 선언하며 자료형 선언 위에 선언
//이클립스에서는 다른 패키지에 작성된 자료형을 사용할 경우 import 명령을 자동 완성하는 기능 제공-[Ctrl]+[Space]=>같은 이름의 자료형이 여러개인 경우 선택
