...
//2.Object.getClass() 메소드를 호출하여 현재 사용중인 클래스에 대한 Class 객체를 반환받는 방법 - JVM이 만들어준 클래스 객체 필요시
...
public class StaticBlockApp {
	public static void main(String[] args) throws ClassNotFoundException {
		/*
		//1.JVM은 ClassLoader 프로그램을 이용하여 클래스(Class 파일)를 읽어 메모리(Method 영역)에 저장
		//2.new 연산자로 메모리에 저장된 클래스(Class 객체 - Clazz)의 생성자를 호출하여 객체(Object - instance) 생성
		...
		*/
		...
		//Class.forName(String className): 문자열로 표현된 패키지가 포함된 클래스를 전달받아 ClassLoader 프로그램을
		//사용하여 클래스를 읽어 메모리에 저장하는 메소드
	    // =>Class 객체(Clazz) 반환
