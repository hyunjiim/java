...
public class SelectStudentApp {
	public static void main(String[] args) {
		...
		try {
			...
			//ResultSet 객체에 저장된 모든 검색행을 행단위로 처리하기 위해 ResultSet 객체는 커서(Cursor) 제공
			...
			//=>true 반환: ResiltSet 커서 위치에 처리행이 있는 경우의 반환값
			if(rs.next()) { //ResultSet 커서 위치에 처리행이 있는 경우
				//System.out.println("[메세지]검색된 학생정보가 있습니다.");
				
				//검색된 다수의 학생정보가 저장된 ResultSet 객체를 처리하기 위한 반복문
				//=>ResultSet 객체에 저자오딘 검색행의 갯수가 불확실하므로 while 구문 사용
				//=>if 구문에서 ResultSet 커서를 다음행으로 이미 이동했으므로 do~while 구문 사용 
				do {
