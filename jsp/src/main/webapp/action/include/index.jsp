...
<%
	...
	//전달값이 있으면 전달값을 구분하여 변수에 파일의 경로 저장
...
<body>
	...
	<%-- => CSL(HTML, CSS, JavaScript) 및 SSL(Java - Script Element)의 소스코드 포함 가능 --%>
	<%-- => JSP 문서를 요청한 경우 include Directive의 file 속성값으로 설정된 외부파일의 
	소스코드를 JSP 문서에 포함한 후 실행한 결과를 클라이언트에게 전달하여 응답 --%>
	...
	<%-- 형식) <jsp:include page="JSP 문서의 경로"/> --%>
	<%-- => page 속성값으로 설정된 JSP 문서가 없는 경우 에러 발생 --%>
	<%-- => page 속성값으로 설정된 JSP 문서가 변경돼도 요청 JSP 문서에 미영향 --%>
	<%-- include Tage의 page 속성값으로 JSP의 표현식(Expression) 사용 가능 --%>
