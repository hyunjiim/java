...
<body>
	...
	<%-- NVC에서 중요하게 다뤄질 내용 --%>
	<p><span>request Scope </span>: request 내장객체를 이용하여 객체를 속성값으로 저장
	- 객체를 속성값으로 저장한 JSP 문서와 스레드가 이동된 JSP 문서에서 속성값을 객체로 
	반환받아 사용 가능</p>
	<%-- JSP 문서 밖에 만들어져 클라이언트의 JSESSIONID 쿠키로 바인딩 처리되는것 - 클라이언트마다 세션이 다름
	 - 클라이언트만 같으면 모든 JSP 문서에서 세션에 저장된 속성값을 제공받아 사용할 수 있음 --%>
	<p><span>session Scope </span>: session 내장객체를 이용하여 객체를 속성값으로 저장
	- 같은 세션이 바인딩된 모든 JSP 문서에서 속성값을 객체로 반환받아 사용 가능</p>
	<%-- application은 WAS에 의해 JSP 문서 밖에 만들어지며, application은 단 하나 --%>
