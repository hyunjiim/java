...
<%-- => [login_form.jsp] 문서의 form 태그를 이용하여 post 방식으로 요청하는 JSP 방식 --%>
<%-- ◈인증 실패: 전달받은 인증정보가 저장맻체에 저장된 인증정보와 일치하지 않은 경우(아이디와 비밀번호가 다른 경우) --%>
<%-- => 클라이언트에게 [login_form.jsp] 문서를 요청할 수 있는 URL 주소를 전달하여 응답 --%>
<%-- ◈인증 성공: 전달받은 인증정보가 저장매체에 저장된 인증정보와 일치하는 경우(아이디와 비밀번호가 같은 경우) --%>
<%-- => 세션에 권한 관련 정보가 저장된 객체를 저장 - 사용자에게 권한 제공: 로그인 --%>
<%-- => 클라이언트엑 [login_user.jsp]문서를 요청할 수 있는 URL 주소를 전달하여 응답 --%>
<%
	//request.getMethod(): JSP 문서를 요청한 요청방식(GET or POST)을 반환하는 메소드
	if(request.getMethod().equals("GET")){
		/*
		//response.senError(int sc): 클라이언트에게 에러코드(4XX or 5XX)를 전달하여 응답 처리하는 메소드
