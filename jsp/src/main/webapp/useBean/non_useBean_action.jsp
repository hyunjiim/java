...
forward 이동하는 JSP 문서 - 클라이언트 요청에 대한 처리만 제공하는 JSP 문서 --%>
<%
	...
	*/
	...
	//JavaBean(객체)을 request 내장객체의 속성값으로 저장
	// => 스레드가 이동되는 JSP 문서만 속성값 제공
	request.setAttribute("hewon", hewon);
	
	//포워드 이동 : 요청 JSP 문서에서 응답 JSP 문서로 스레드를 이동하여 응답처리
	...
	//RequestDispatcher.forward(request, response): 포워드 이동하는 메소드 - forward 태그와 동일
	// => 매개변수에 request, response 객체를 저장하여 스레드가 이동되는 JSP 문서에 전달
	request.getRequestDispatcher("non_useBean_display.jsp").forward(request, response);
	
%>
<%-- forward 태그 대신 RequestDispatcher.forward() 메소드를 통해 포워드 이동 가능 --%>
