...
// => 방명록 게시글의 [변경] 태그를 클릭한 경우 방명록 게시글 변경페이지(/guest/modifyForm.itwill)로 이동
// => 방명록 게시글의 [삭제] 태그를 클릭한 경우 방명록 게시글 삭제
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(guestList.isEmpty()) {//List 객체에 요소가 없는 경우 - 검색행이 없는 경우
			...
			for(GuestDTO guest : guestList) {
				...
				out.println("<table border='1' cellspacing='0' width='100%'>");
				out.println("<tr>");
				out.println("<th width='150'>번호</th>");
				out.println("<td width='150' align='center'>"+guest.getNum()+"</td>");
				...
				out.println("<td width='650' colspan='5'>"+guest.getSubject()+"</td>");
				out.println("</tr>");
				out.println("<tr>");
				out.println("<th width='150'>내용</th>");
				out.println("<td width='650' colspan='5'>"+guest.getContent().replace("\n", "<br>")+"</td>");
				out.println("</tr>");
				out.println("<tr>");
