...
//클라이언트 요청에 의해 이미지 파일로 응답하기 위한 서블릿 - response 몸체부에 이미지파일을 넣어 전송
...
public class SendImageServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//jpg 파일을 보낼 경우: image/jpeg, png 파일을 보낼 경우: image/png
		// => 응답파일이 문서파일이 아니므로 캐릭터셋 미설정
		response.setContentType("image/png");
		
		//응답파일을 생성하기 위한 출력스트림을 반환받아 저장
		// => 이미지 파일(이진 파일: Binary File)을 생성하기 위해 원시데이터(1Byte)를 전달하는
		...
		//파일 시스템 경로를 반환하는 메소드
		String imageFilePath=request.getServletContext().getRealPath("/WEB-INF/Koala.png");
		//System.out.println("imageFilePath = "+imageFilePath); //C:\Users\ user\Downloads\apache-tomcat-9.0.75\webapps\servlet\WEB-INF\Koala.png
