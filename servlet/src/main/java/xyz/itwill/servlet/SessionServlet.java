...
// => 세션을 구분하기 위한 식별자(SessionID)를 이용하여 클라이언트를 구분해 세션에 값(객체)을
//저장하여 사용
...
//세션 트랙킹(Session Tracking): 클라이언트에게 제공받은 [JSESSIONID] 이름의 쿠키값과 서버 메모리에
//저장된 세션의 식별자(SessionId)를 비교하여 검색하는 작업
...
public class SessionServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		out.println("<hr>");
		//HttpSession.isNew(): 세션을 트랙킹하여 바인딩된 경우 [false]를 반환하고 세션을
		...
		// => HttpSession 객체에 연결 지속성을 제공하기 위한 값(객체)을 저장하는 메소드
		...
		//HttpSession.invalidate(): 바인딩된 세션을 언바인딩하여 무효화처리하는 메소드
