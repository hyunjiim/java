...
//@(Annotation): API 문서에 특별한 설명을 제공하기 위해 만들어진 기능
...
// => name 속성: 서블릿의 이름을 속성 값으로 설정 - 속성을 생략하면 자동으로 클래스 이름을
//	서블릿의 이름으로 설정 
// => value 속성: 서블릿을 요청하기 위한 URL 패턴을 속성값으로 설정(생략 불가능) - value 속성외
//	다른 속성이 없는 경우 속성값만 설정 가능 
//@WebServlet(name="first", value="/first.itwill")  
@WebServlet("/first.itwill") //실행에 필요한 정보 제공 
public class FirstServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//요청에 대한 처리 명령 작성 및 처리결과를 저장할 응답파일 생성
		// => 서블릿은 모든 클라이언트에게 일관성 있는 처리결과를 동적으로 생성하여 제공
		Date now=new Date(); //서버 플랫폼의 현재 날짜와 시간이 저장된 Date 객체 생성
		
		//날짜와 시간에 대한 패턴정보를 저장한 SimpleDataFormat 객체 생성
		SimpleDateFormat dateFormat=new SimpleDateFormat("yyyy년 MM월 dd일 HH시 mm분 ss초");
		
		//SimpleDataFormat 객체의 format() 메소드를 호출하여 매개변수로 전달받은 Date 객체를
		//SimpleDataFormat 객체에 저장된 날짜와 시간 패턴의 문자열로 변환하여 반환
		String displayNow=dateFormat.format(now);
		
		//실행결과 클라이언트에게 보내는 방법
		//실행결과를 저장한 응답파일 생성 - HTML
