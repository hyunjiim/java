...
//[multipart/form-data]로 전달된 값과 파일은 request 객체로부터 입력스트림(ServletInputStream 객체)을
//제공받아 임시파일에 저장한 후 전달값과 전달파일을 구분하여 처리
// => [multipart/form-data]를 처리하는 클래스를 사용하는 것을 권장
...
// 2. Oreilly 그룹에서 배포한 cos 라이브러리의 클래스 사용 - 입력값 검증 없이 무조건 파일 업로드 
...
public class UploadServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		*/
		...
		/*
		//getInputStream() 메소드는 [multipart/form-data]를 읽어들여 [multipart/form-data]를 저장하기 위한 임시파일 생성
		// => 정보를 읽어들여 분리 저장하는 과정이 필요 >> 복잡하고 어려움
		...
		*/
		...
		//[submit 버튼을 누른 후]: saveDirectory = C:\Users\ user\Downloads\apache-tomcat-9.0.75\webapps\servlet\ upload
		...
		//[, String encoding][, FileRenamePolicy policy]) 생성자를 사용하여 객체 생성
		...
		//FileRenamPolicy 객체: 업로드될 파일의 이름과 같은 이름의 파일이 서버 디렉토리에 이미 존재할 경우
