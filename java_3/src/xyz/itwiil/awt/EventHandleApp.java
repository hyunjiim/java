...
//=>Listener 인터페이스를 상속받아 자식클래스에서 추상메소드를 오버라이드 선언하여 오버라이드 선언된 메소드에 이벤트 처리 명령 작성

//3.이벤트 소스에서 이벤트가 발생되면 이벤트 처리 클래스의 객체를 제공받아 이벤트 처리 명령이 실행되도록 설정
//=>Component.addXXXListener(Listener 1)메소드 호출
//=>Component.addXXXListener(Listener 1): 컴퍼넌트의 이벤트를 처리하기 위한 이벤트 처리 객체 등록

//[EXIT]버튼을 누른 경우 프로그램을 종료하는 기능을 제공하는 프로그램 작성
public class EventHandleApp extends Frame {
	...
	public EventHandleApp(String title) {
		...
		//컴퍼넌트에 이벤트 처리 객체 등록
		//=>Button 컴퍼넌트에서 ActionEvent가 발생될 경우 EventHanble 클래스의 객체를 제공받아 이벤트 처리 메소드를 자동 호출하여 이벤트 처리
		exit.addActionListener(new EventHandle()); //이벤트큐?스레드
