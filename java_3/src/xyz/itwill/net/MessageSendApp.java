...
public class MessageSendApp {
	public static void main(String[] args) throws IOException {
		...
		//DatagramSocket 클래스: 다른 컴퓨터와 연결하기 위한 정보를 저장하기 위한 클래스(어떤 컴퓨터랑 연결될 것인가?)
		//=>데이터를 보내는 컴퓨터는 기본생성자를 사용하여 DatagramSocket 객체 생성
		...
		//=>DatagramPacket 클래스의 DatagramPacket(byte[] data,int length, InetAddress,int port)생성자를 사용하여 데이터를 보내기 위한 패킷 정보가 저장된 DatagramPacket 객체 생성
		DatagramPacket packet=new DatagramPacket(data,data.length,address,4000);
		
		//DatagramPacket.send(DatagramPacket packet): 매개변수로 전달받은 DatagramPacket 객체의 패킷정보를 이용하여 데이터(패킷)를 전달하는 메소드
		socket.send(packet);
