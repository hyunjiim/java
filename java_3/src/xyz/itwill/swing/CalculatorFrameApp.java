...
public class CalculatorFrameApp extends JFrame implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		JButton만 있으므로 다음과 같이 사용할 수 있음 
		JButton 컴퍼넌트의 메소드를 사용하기 위해 이벤트가 발생된 JButton 컴퍼넌트를 반환받아 저장
		...
		*/
		...
		if(eventButton==b_C) { 	//이벤트가 발생된 버튼이 [C]인 경우
			operation="";
			label.setText("0");
		}else if(eventButton==b_equals) {	//이벤트가 발생된 버튼이 [=]인 경우
			//연산식에서
			...
			try {
				...
				//JOptionPane 클래스: 다이얼로그를 제공하기 위한 클래스
				//JOptionPane.showMessageDialog(Component parent,String message): 메세지 다이얼로그를 출력하는 메소드
