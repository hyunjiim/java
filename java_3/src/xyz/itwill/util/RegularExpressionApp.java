...
//정규표현식(RegularExpression): 메타문자(Meta Character), 회피문자(Escape Character)등을 사용하여 일정한 규칙의 문자열을 표현하는 방법-사용자 입력값에 대한 검증
/*
 ...
 * ⑦ \메타문자: 메타문자를 일반 문자로 표현됨을 의미 -ex)\. : .문자
 */
...
public class RegularExpressionApp {
	public static void main(String[] args) {
		...
		//아이디 형식(패턴): 영문자로 시작(^)되며 영문자, 숫자, 특수문자(_)의 조합(\w)으로 6~20범위의 문자들로 구성된 문자열
		...
		//String idReg="^[a-zA-Z][a-zA-Z0-9_]{5,19}$";  //6~20범위인데 왜 {5,19}?
		String idReg="^[a-zA-Z]\\w{5,19}$";  //\를 회피문자로 인식하기 때문에 \\w로 사용해야 \w문자로 인식
		
		//Pattern 클래스: 정규표현식을 사용하기 위한 기능의 메소드를 제공하는 클래스
		//Pattern.matches(String regEx, charSequence input): 정규표현식과 입력값을 비교하여 정규표현식의 패턴과 입력값이 다르면 [false]를 반환하고 같으면 [true]를 반환하는 메소드
		...
		*/
		...
		//비밀번호 형식(패턴): 영문자, 숫자, 특수문자(_)가 반드시 1번이상 포함되며 8~30 범위의 문자들로 구성된 문자열
		...
		//(?=.*[~!@#$%^&*-_]) =>Error 발생 -가 중간에 삽입되어 있을 경우 범위를 나타냄
		//(?=.*[^~!@#$%&*-_]) =>Error 발생 ^가 맨 앞에 삽입되어 있을 경우 문자열이 ~로 시작되어야 함을 나타냄
		String passwordReg="(?=.*[a-zA-Z])(?=.*[0-9])(?=.*[~!@#$%^&*_-]).{8,30}$";  //{}앞에 .이 필요한 이유?
