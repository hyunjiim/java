...
<body>
	...
	<ul>
		<%-- EL 표현식에서는 스코프 객체에 상관없이 속성명을 사용하여 속성값을 제공받아 출력 가능 --%>
	...
	<ul>
		...
		Scope 객체의 속성값을 제공받아 출력 처리 - Scope 객체의 속성명은 다르게 설정하는 것을 권장 --%>
	...
	<h2>스코프 객체의 속성명이 같은 다른 경우 - EL 내장객체 사용</h2>
	<ul>
		...
		<li>pageName 속성값(Page Scope) = ${pageScope.name }</li> <%-- 홍길동 --%>
		<li>requestName 속성값(Request Scope) = ${requestScope.name }</li> <%-- 홍길동 --%>
		<li>sessionName 속성값(Session Scope) = ${sessionScope.name }</li> <%-- 홍길동 --%>
		<li>applicationName 속성값(Application Scope) = ${applicationScope.name }</li> <%-- 홍길동 --%>
	</ul>
</body>
</html>