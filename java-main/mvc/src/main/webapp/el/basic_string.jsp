...
<%
	...
	// => pageContext 내장객체에 속성명(문자열)과 속성값(객체)를 전달받아 저장하는 메소드 - page
	//page Scope: 스코프 속성값(pageContext 내장객체에 저장된 속성값)을 저장한 웹프로그램에서만 객체를 반환받아 사용가능
...
<body>
	...
	<%-- if 구문을 사용하여 반환받은 객체가 [null]이 아닌 경우에만 JSP 표현식으로 출력처리 --%>
	<p>이름(pageName) = <%if(pageName!=null){ %><%=pageName %><% } %></p>
	<hr>
	<h2>EL 사용</h2>
	<%-- ${속성명}: EL 표현식으로 스코프 객체의 속성명을 사용하면 속성값을 제공받아 출력처리 --%>
	<%-- => getAttribute() 메소드를 호출하지 않아도 스코프 속성값을 제공받아 출력처리 가능 --%>
