...
<body>
	<h1>Formatter - 날짜와 시간의 변환</h1>
	<hr>
	<%-- 서버 시스템의 현재 날짜와 시간이 저장된 Date 객체를 생서아형 스코프 객체의 속성값으로 저장 --%>
	<c:set var="now" value="<%=new Date() %>"/>
	<%-- EL을 이용하여 Date 객체를 제공받아 출력 처리할 경우 toString() 메소드 자동 호출 --%>
	<p>now = ${now }</p>
	
	<%-- formatData 태그: Date 객체에 저장된 날짜와 시간을 원하는 패턴의 문자열로 변환하여 제공하는 태그 --%>
	<%-- => SimpleDataFormat 클래스의 format() 메소드와 유사한 기능 제공 --%>
	<%-- value 속성: Date 객체를 속성값으로 설정 - EL을 사용하여 Date 객체를 제공받아 속성값으로 사용 --%>
	<%-- type 속성: date(날짜), time(시간), both(날짜와 시간) 중 하나를 속성값으로 설정 --%>
	<%-- => type 속성값을 [data]로 설정한 경우 기본적으로 [yyyy.M.d.] 패턴의 문자열로 변환되어 제공 --%>
	<p>now(날짜) = <fmt:formatDate value="${now }" type="date"/></p>
	
	<%-- dataStyle 속성: full 또는 short 중에 하나를 속성값으로 설정 --%>
	...
	<%-- type 속성: date(날짜), time(시간), both(날짜와 시간) 중 하나를 속성값으로 설정 --%>
	<%-- => type 속성값을 [time]으로 설정한 경우 기본적으로 [a h:mm:ss] 패턴의 문자열로 변환되어 제공 --%>
	<p>now(시간) = <fmt:formatDate value="${now }" type="time"/></p>
	
	<%-- timeStyle 속성: full 또는 short 중에 하나를 속성값으로 설정 --%>
	<%-- => timeStyle 속성값이 [full]인 경우 [a h시 mm분 ss초 z] 패턴의 문자열로 반환되어 제공 --%>
