...
Core 태그 라이브러리의 커스텀 태그를 제공받아 사용 가능 --%>
...
<body>
	<h1>Core - EL 지원 태그</h1>
	<hr>
	<!-- set 태그: 스코프 객체의 속성값으로 객체를 저장(변경)하기 위한 태그 - setAttribute와 비슷한 역할 -->
	<!-- var 속성: 스코프 객체의 저장될 속성값을 구분하기 위한 속성명을 속성값으로 설정 -->
	<!-- value 속성: 스코프 객체의 저장될 속성값을 속성값으로 설정 -->
	<!-- scope 속성: 스코프 객체의 저장된 속성값의 사용범위를 태그 속성값으로 설정 -->
	<!-- => page, request, session, application 중 하나를 속성값으로 설정 -->
	<!-- => scope 속성을 생략하면 [page] 속성값을 기본값으로 사용 -->
	<c:set var="su" value="10" scope="page"/>
	<p>su = ${su }</p>
	
	<%-- value 속성대신 태그내용을 사용하여 스코프 객체에 저장될 속성값 설정 가능 - 비권장 --%>
	<%-- 스코프 객체의 속성명이 같은 경우 스코프 객체의  속성값 변경 처리 --%>
	<c:set var="su">20</c:set>
	<p>su = ${su }</p>
	
	<%-- set 태그의 value 속성값으로 EL을 이용하여 객체를 제공받아 속성값으로 저장 가능 --%>
	...
	<%-- EL 표현식으로 제공받은 스코프 객체의 속성값이 Java 객체인 경우 자동으로 toString()
	메소드를 호출하여 반환받아 문자열로 출력 처리 --%>
	<p>now = ${now }</p>
	<%-- EL 표현식은 원래 Getter 메소드를 호출하기 때문에 Getter 메소드를 호출할 필요없이 time만
	작성하여 똑같은 결과를 얻을 수 있음 --%>
	...
	toString() 메소드 호출 - 객체의 메모리 주소(HashCode)를 문자열로 반환받아 출력 처리 --%>
	<p>student = ${student }</p>
	<%-- EL 표현식에 의해 Java 객체의 필드값을 제공받아 출력 처리 - Getter 메소드 자동 호출 --%>
	<p>학번 = ${student.num }, 이름 = ${student.name }</p>
	
	<%-- set 태그를 사용하여 스코프 객체에 저장된 속성값(객체)의 필드값 변경 가능 --%>
