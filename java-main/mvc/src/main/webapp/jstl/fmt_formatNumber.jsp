...
<body>
	...
	<%-- => 속성값으로 제공받은 값이 숫자값이 아는 경우 NumberFormatException 발생 --%>
	...
	<%-- => type 속성값을 [currency]로 설정한 경우 앞부분에 화폐단위를 출력하고 숫자 3자리 마다 [,] 기호를
	삽입한 문자열로 변환하여 제공 --%>
	<p>가격 = <fmt:formatNumber value="${price }" type="number"/>원</p>
	<p>가격 = <fmt:formatNumber value="${price }" type="currency"/></p>
	
	<%-- setLocal: 장소를 변경하는 태그 - 나라별로 날짜와 시간 또는 화폐단위를 변경 --%>
