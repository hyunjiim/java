...
public class AnnotationStudentServiceImpl implements StudentService {
	...
	// => Setter 메소드를 이용하여 의존관계를 구현하지만 Setter 메소드를 선언하지 않아도 의존성 주입 가능
	
	//문제점)필드의 자료형과 같은 자료형의 Spring Bean이 2개 이상인 경우 의존성 주입 실패 - NoUniqueBeanDefinitionException 발생 - ??????
	// => Annotation
	// => https://haruhiism.tistory.com/182 참고
	...
	//aurowire 속성값을 [byName]으로 설정한 것과 동일한 방법으로 의존성 주입
	
	//해결법-2)필드의 자료형과 같은 자료형의 Spring Bean이 2개 이상인 경우 필드에 저장될 Spring
	//Bean의 클래스에 @Primary 어노테이션을 사용하여 의존성 주입 - 우선권 제공하는 어노테이션으로
	//여러개 사용 시 에러 발생(우선권을 2개 이상의 Spring Bean에 부여할 경우)
