...
public class AopAnnotationAdvice {
	...
	// => 다른 속성이 없는 경우 속성값만 설정 가능 - within : 포함되는 모든 메소드
	...
	public void afterLog() {
		log.info("[after]핵심관심코드 실행 후 반드시 삽입되어 실행될 횡단관심코드");
	}
	...
	public void afterReturningLog(Object object) {
		log.info("[after-returning]핵심관심코드가 정상적으로 실행된 후 삽입되어 실행될 횡단관심코드");
	}
	...
	public void afterThrowingLog(Exception exception) {
		log.info("[after-throwing]핵심관심코드 실행시 예외가 발생되면 삽입되어 실행될 횡단관심코드");
	}
	...
	public Object aroundLog(ProceedingJoinPoint joinPoint) throws Throwable {
		log.info("[around]핵심관심코 실행 전에 삽입되어 실행될 횡단관심코드");
		Object result=joinPoint.proceed();
		log.info("[around]핵심관심코 실행 후에 삽입되어 실행될 횡단관심코드");
