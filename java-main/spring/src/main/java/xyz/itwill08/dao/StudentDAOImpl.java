...
//SpringDAO 기능을 이용하여 DAO 클래스 작성 - spring-jdbc 라이브러리를 프로젝트에 빌드 처리 : pom.xml
// => JdbcTemplate 객체의 메소드를 호출하여 DAO 클래스의 메소드 작성

//일반적인 JDBC 보단 훨씬 간편하지만 MyBatis를 사용하는 것이 더 간편
...
public class StudentDAOImpl implements StudentDAO {
	...
	public Student selectStudent(int no) {
		...
		//RowMapper 객체 : 검색행을 Java 객체로 변환하기 위한 매핑정보가 저장된 객체
		// => 검색행의 컬럼값을 Java 객체 필드에 저장하기 위한 매핑정보 제공
		// => RowMapper 인터페이스를 상속받은 자식클래스로 객체 생성 - 익명의 내부클래스로 객체 생성 가능
		// => RowMapper 객체를 생성할 때 사용되는 제네릭에는 변환할 Java 객체의 자료형을 설정
