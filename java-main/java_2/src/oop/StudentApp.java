...
public class StudentApp {
	public static void main(String[] args) {
		...
		Student[] students= {new Student(1000, "홍길동", 90, 90), new Student(2000, "임꺽정", 94, 98)
							, new Student(3000, "전우치", 91, 80), new Student(4000, "일지매", 76, 82) 
							, new Student(5000, "장길산", 84, 86)};
		
		//int total=0;  //모든 학생들의 점수 합계를 저장하기 위한 변수
		
		//배열의 참조요소에 저장된 객체의 메모리 주소를 차례대로 제공받아 변수에 저장하여 처리하는 향상된 for 구문을 사용하여 일괄처리
		//NullPointException이 발생하지 않음
		for(Student student : students) {
			student.display();
			//total+=student.getTot();
			
			//정적 필드는 public 접근 제한자로 설정된 경우 클래스를 이용하여 접근해 사용 가능
