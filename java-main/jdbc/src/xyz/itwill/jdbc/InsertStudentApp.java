...
/*JDBC(Java DataBase Connectivity): Java로 DBMS 서버에 접속해 SQL 명령을 전달하여 실행하기 위한 기능을 제공하는 
 * Java API(클래스 또는 인터페이스)
 * 
 * java.sql: JDBC 기능의 프로그램을 작성하기 위한 클래스 및 인터페이스가 선언된 패키지
 * =>java.sql 패키지에서는 JDBC 기능 구현을 위해 인터페이스 제공 - DBMS 종류가 다양하므로 클래스로 제공 불가능
 * =>인터페이스는 객체로 만들 수 없어 DBMS 프로그램을 관리하는 그룹에서 JDBC 기능을 구현하기 위한 클래스(JDBC Driver)를
 *  배포하여 제공
 * =>JDBC Driver가 포함된 라이브러리 파일(Jar 파일)을 다운로드 받아 프로젝트 빌드처리 해야만 라이브러리의 클래스를
 *  사용하여 JDBC 프로그램 작성 가능
 * 
 ...
 *  =>프로젝트 >> 마우스 오른쪽 버튼 >> Properties(속성) >> Java Build Path(메뉴) >> Libraries(Tab) >> classpath 선택
 *    >> ADD Jars 선택 >> 프로젝트 내부의 Jar(ojdbc11.jar) 선택 >> Apply And Close
 *
 */
...
public class InsertStudentApp {
	public static void main(String[] args) {		
		...
		Statement stmt=null;
		try {
			//1.OracleDriver 클래스로 객체 생성 >> DriverManager 클래스의 JDBC Driver 객체로 등록
			// =>OracleDriver 클래스를 읽어 메모리에 저장하여 자동으로 OracleDriver 객체를 생성하여 DriverManager 클래스의 JDBC Driver 객체 등록
			...
			// =>DML 명령의 실행결과로 조작행의 갯수가 저장된 정수값을 반환
			...
			//String sql="insert into student values(2000,'임꺽정','010-7894-5612','수원시 월정구','00/01/02')";
			String sql="insert into student values(3000,'전우치','010-1478-2589','인천시 성당구','00/01/03')";
		...
		}finally{//예외 발생과 상관없이 무조건 실행되는 명령을 작성하기 위한 영역
			try{
				//6. JDBC 관련 객체를 모두 제거 - 객체가 생성된 반대로 제거
				//Statement,close(): Statement 객체를 삭제하는 메소드
				// =>NullPointException이 발생할 수 있으므로 예외 발생을 방지하기 위해 if 구문 사용
				// NullPointException: 참조변수에 NULL이 저장된 상태에서 메소드를 호출한 경우 발생되는 예외
