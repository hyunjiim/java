...
public class Method {
	// 반환형: 없음 메소드명:displayOne 매개변수: 없음
	void displayOne() {
	...
	void printTwo(int num) {
		// 매개변수에 저장된 값에 대한 검증
		if (num <= 0) {
			System.out.println("[입력오류]0보다 큰 값이 입력되어야 합니다.");
			return;
		}
	...
	void printThree(int num1, int num2) {
		...
		/*
		 * if(num1>=num2) { 
		 * 		System.out.println("[입력오류]처음 입력한 값이 다음에 입력된 값보다 작아야합니다.");
		 * 		return; 
		 * }
		 */

		if (num1 == num2) {
			System.out.println("[입력오류]서로 다른 정수값이 입력되어야 합니다.");
			return;
		}
