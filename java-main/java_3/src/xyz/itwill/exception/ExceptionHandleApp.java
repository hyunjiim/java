...
//=>프로그램 실행시 예외가 발생되면 예외가 발생된 지점에서 프로그램이 강제 종료
...
//예외의 종류
//1.일반 예외: 프로그램 실행시 예외가 발생될 가능성이 높아 컴파일시 예외처리를 하지 않은 경우 에러 발생 
...
//	- 참조변수에 저장된 예외관련 객체의 메소드를 호출하여 예외 처리
...
//finally 키워드의 블럭에는 예외 발생과 상관없이 무조건 실행될 명령 작성- 생략가능
//=>사용 자원에 대한 제거 명령 실행
...
public class ExceptionHandleApp {
	public static void main(String[] args) {
		...
		try{
			...
			//=>원래는 Throwable.getStackTrace()를 사용해 예외 발생 경로를 역추적해 기록하는 것이 맞음
			e.printStackTrace();
		}finally {
