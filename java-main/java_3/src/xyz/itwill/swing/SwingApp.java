package xyz.itwill.swing; //-1번째 클래스
...
//=>JFrame.setDefaultCloseOperation(int operation) 메소드를 호출하여 프레임의 [닫기]를 누른 경우 동작되는 기능을 변경 가능
...
//=> DISPOSE_ON_CLOSE: 메모리를 정리하고 프레임을 종료하는 상수 
//=> EXIT_ON_CLOSE : 프로그램을 종료하는 상수 

//3.프레임을 직접 변경하지 않고 프레임의 Container 객체를 반환받아 변경 처리
//=>프레임의 배치관리자 변경, 프레임의 배경색 변경, 프레임의 컴퍼넌트 부착 등

//JTextField 컴퍼넌트에서 메세지를 입력 후 [Enter]를 누르면 JTextArea 컴퍼넌트에 메세지를 추가하여 출력되도록 프로그램 작성
...
public class SwingApp extends JFrame {
	...
	public SwingApp(String title) {
		...
		//JTextArea 컴퍼넌트에 입력초점을 제공하지 않도록 설정-출력 컴퍼넌트로만 사용
		jTextArea.setFocusable(false);
		
		//JTextArea.getContentPane(): 프레임의 컨테이너 기능을 객체로 반환하는 메소드
		...
		//프레임의 [닫기]버튼을 누른 경우 프로그램을 종료하는 기능
	...
	public class TextEventHandle implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if(!text.equals("")) {//입력된 문자열이 있는 경우
				...
				jTextArea.setCaretPosition(jTextArea.getText().length()); //스크롤을 맨 밑으로 이동
