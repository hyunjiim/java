...
//java.text 패키지: 값을 변환하는 기능을 메소드로 제공하기 위한 클래스

//SimpleDateFormat 클래스: Date 객체와 String 객체를 서로 변환하기 위한 기능을 메소드로 제공하는 클래스
public class SimpleDateFormatApp {
	public static void main(String[] args) {
	    ...
	    //=> 패턴 문자 : y(년),M(월),d(일),E(요일),AM or PM(오전 또는 오후),h(시:12),H(시:24),m(분),s(초)
		SimpleDateFormat dateFormat=new SimpleDateFormat("yyyy년 MM월 dd일 E요일");
		
		//SimpleDateFormat.format(Date date): Date 객체에 저장된 날짜와 시간을 SimpleDateFormat객체에 저장된 패턴정보를 사용하여 문자열(String 객체)로 변환하여 반환하는 메소드
		String prinDate=dateFormat.format(now);
		
		System.out.println("현재 날짜: "+prinDate);
		...
		//패턴정보를 변경하는 메소드
	      ...
	      try {
	         ...
	         //                        전달받아 원하는 날짜와 시간이 저장된 Date 객체를 생성하여 반환하는 메소드
	         //=> SimpleDateFormat 객체에 저장된 패턴정보와 일치하지 않는 문자열이 전달된 경우 ParseException 발생 가능 - 예외처리
	         Date wantDate=dateFormat.parse(want); //String객체 >> Date 객체
