...
//   (main 스레드에서 작동X=>main 메소드에 작성하면 안됨)
//=>클라이언트에게 동일한 메세지를 전달하기 위해 소켓 스레드를 컬렉션에 저장해 사용할 수 있도록 설정
...
public class ChatServerApp {
	...
	public class SocketThread extends Thread {
		...
		// =>클라이언트가 보내온 메세지를 전달받아 모든 클라이언트에게 전달하는 명려여 작성
		@Override
		public void run() {
			String aliasName = ""; // 클라이언트의 대화명을 저장하기 위한 변수 선언

			try {
				// 소켓의 입력스트림을 제공받아 대량의 문자데이터를 읽을 수 있는
				// 입력스트림으로 확장하여 필드에 저장
				in = new BufferedReader(new InputStreamReader(client.getInputStream()));

				// 소켓의 출력스트림을 제공받아 문자열을 전달할 수 있는
				// 출력스트림으로 확장하여 필드에 저장
				// =>PrintWriter 클래스의 PrintWriter(OutputStream out, boolean autuFlush)
				// 생성자를 사용하여 PritWriter 객체 생성
				// =>autuFlush 매개변수에 [true]를 전달하면 버퍼를 사용하지 않고 무조건
				// 출력스트림으로 데이터를 전달
				out = new PrintWriter(client.getOutputStream(), true);// true 작성하지 않으면 따로 flush() 메소드를 사용해줘야 함
				...
				// List 객체에 저장된 요소(SocketThread 객체)에서 접속중인 클라이언트의 정보 삭제
