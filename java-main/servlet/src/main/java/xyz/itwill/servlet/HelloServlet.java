package xyz.itwill.servlet; //01

//was->post(form태그)/get 방식으로 웹프로그램 요청->클래스 로딩이 안되어 있으면 클래스로더로 
//클래스 로딩한 다음 클래스 객체 생성->doGet/doPost 메소드 호출->실행결과 Response에 담아 반환

//new > dynamic web project > servlet > next > next > generate web.xml deployment descriptor 체크 필수 > finish
...
//=> Servlet 프로그램은 클라이언트의 요청으로 WAS(Web Application Server: 웹서버 확장 버전)에 의해
//실행되며 실행결과로 웹문서를 동적으로 생성하여 클라이언트에게 응답
...
public class HelloServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//=> response 메세지의 몸체부에 저장될 파일 형태 설정
		//HttpServletResponse.setContentType(String mimeType[;charset=encoding]) 메소드 호출
		// => response 메세지의 몸체부에 저장될 파일형태를 변경하는 메소드
		// => 응답 파일이 문서파일인 경우 문자형태(CharacterSet - Encoding) 설정
		//기본 응답 파일 형태 : text/html;charset=iso-8859-1
		response.setContentType("text/html;charset=utf-8"); //한글로 만들어진 HTML 문서로 응답
		
		//4.응답파일을 생성하기 위한 출력스트림을 반환받아 저장
		//HttpServletResponse.getOutputStream(): 응답파일을 생성하기 위한 출력스트림(OutputStram)
		//(OutputStream 객체)를 반환하는 메소드
		// => 원시데이터(1Byte)를 전달해 그림,소리,동영상 같은 이진파일을 생성하기 위한 출력스트림
		//HttpServletResponse.getWriter(): 응답파일을 생성하기 위한 출력스트림(PrintWriter 객체)
		// => 문자데이터(2Byte)를 전달해 HTML, XML 파일등과 같은 문서파일을 생성하기 위한 출력스트림
		PrintWriter out=response.getWriter();
		
		//5.클라이언트 요청에 대한 처리 명령 작성 및 출력스트림을 이용한 응답파일 생성
		// => 클라이언트 요청에 대한 처리 명령에 필요한 값은 HttpServletRequest 객체로부터 제공
		...
		//6.Servlet 클래스를 Servlet 프로그램(웹프로그램)으로 실행되도록 설정
		//=> [web.xml] 파일에서 서블릿 클래스를 서블릿으로 등록하고 서블릿을 요청할 수 있는
		//URL 주소를 매핑 설정
		//클라이언트가 매핑된 URL 주소의 서블릿을 요청하면 WAS 프로그램은 서블릿에 대한 클래스
