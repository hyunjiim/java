<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%-- AJAX를 사용하면 form 태그를 이용하지 않고 데이터 전달 가능 --%>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
...
<body>
	...
	document.getElementById("getBtn").onclick= function() {
		...
		//입력값 검증
		if(name==""){ //name==null 하면 작동되지 않음- why?
		...
		xhr.onreadystatechange= function() {
			if(xhr.readyState==4){
				if(xhr.status==200){ //XMLHttpRequest 객체에 응답결과가 전달된 경우
		...
		//=> GET 방식으로 웹프로그램을 요청할 경우 질의문자열(QueryString)을 사용해 전달
		//문제점)전달값에 URL 주소로 표현 불가능한 문자가 존재할 경우 400에러코드 발생 가능
		//웹키 방식의 브라우저(크롬,엣지)는 상관없는데 다른 웹브로우저에서는 404에러 발생 가능)
