...
내장객체의 속성값으로 저장하여 제공 --%>
<%-- id 속성(구분자): useBean 태그로 제공되는 객체를 구분하기 위한 식별자를 속성값으로 설정 --%>
...
<%-- 
	//useBean 태그와 아래의 코드의 역할 동일 - 객체를 생성하여 내장객체의 속성값으로 저장하여 제공
...
<%-- name 속성: useBean 태그로 제공된 객체의 식별자(id 속성)을 속성값으로 설정 --%>
<%-- property 속성: 값을 변경할 필드명을 속성값으로 설정 --%>
<%-- => 필드명을 기반으로 작성된 Setter 메소드를 자동 호출 - 필드명으로 작성된 Setter 메소드가 없으면 에러 발생 --%>
