...
<body>
	...
	function submitCheck() {
		...
		<%--질문: 아래처럼 작성하여 no 값을 이동 문서에 전달하면? --%>
		<%--studentForm.action="updateStudent.jsp?no=<%=student.getNo()%>";--%>
		<%-- 가능은 하지만 name, phone, address, birthday 하나하나를 모두 부호화처리해야하기 때문에 복잡해짐 --%>
		<%-- => form 태그를 보낼 때에는 위에 작성한 코드처럼 input 태그에 no를 같이 보내주는 것이 좋음 --%>
