<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%-- 전달값을 이용하여 다른 문서로 포워드 이동하는 JSP 문서 --%>
<%
	//전달값을 반환받아 저장
	...
	if(pageName==null){ //전달값이 없는 경우
		...
		//JSP 문서를 요청하여 실행결과(웹문서 - HTML)를 응답받아 출력
...
<%-- forward Tag: 요청 JSP 문서에서 다른 JSP 문서를 스레드를 이동하여 스레드가 이동된
JSP 문서로 응답 처리하기 위한 기능을 제공하는 태그 - 포워드 이동 --%>
<%-- => 서버측에서 스레드를 이동하였으므로 클라이언트의 URL 주소 미변경 --%>
...
<%-- NVC 패턴으로 프로그램을 만들 때 자주 사용하게 될 태그 --%>
<%-- JSP Model1: 일반적인 웹페이지, JSP Model2(NVC): 고급진 웹페이지 --%>
