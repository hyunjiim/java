...
public interface MyHewonInterfaceMapper {
	...
	//XML 기반의 매퍼 파일에선 resultMap을 사용하여 매핑정보를 재사용하는 것이 가능했지만
	//@Results 어노테이션의 매핑정보는 다른 추상메소드에서 재사용 불가능 - 유지보수의 효율성 
	//@ResultMap 어노테이션을 사용하면 재사용 가능하지만 어노테이션을 사용하기 위해 XML 기반의
	//매핑 파일을 따로 생성해줘야 함 - 굳이...
	@Results(value = {
			@Result(column = "hewon_id", property = "id")
			,@Result(column = "hewon_name", property = "name")
			,@Result(column = "hewon_phone", property = "phone")
